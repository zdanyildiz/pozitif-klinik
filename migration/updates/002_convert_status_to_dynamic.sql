-- 1. Önce statü tablosunu oluştur
...
-- 2. Varsayılan verileri ekle (Mevcut ENUM değerleri + İstenen yeniler)
...
-- 3. cln_appointments tablosundaki status alanını ENUM'dan VARCHAR'a çevir
-- Bu işlem mevcut verileri korur ancak ENUM kısıtlamasını kaldırır.
ALTER TABLE cln_appointments MODIFY COLUMN status VARCHAR(50) NOT NULL DEFAULT 'pending';

-- Opsiyonel: Eski foreign key veya constraint varsa temizlenebilir ama şu an ENUM olduğu için gerek yok.
