...
CREATE TABLE `cln_services` (
  ...
  `code` varchar(50) DEFAULT NULL COMMENT 'Internal Service Code',
...
CREATE TABLE `cln_appointment_items` (
  ...
  `description` varchar(255) DEFAULT NULL COMMENT 'Kalem bazlı açıklama/not',
...
CREATE TABLE `cln_examinations` (
  ...
  `specialty_data` LONGTEXT DEFAULT NULL COMMENT 'JSON storage for dynamic specialty fields',
  `created_at` datetime NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `clinic_id` (`clinic_id`),
  KEY `patient_id` (`patient_id`),
  ...
  KEY `appointment_id` (`appointment_id`),
  KEY `idx_exam_legacy_visit_id` (`legacy_visit_id`),
